...
--
-- Generated by tools/build-bluebox.mjs from https://github.com/ryanbooz/bluebox
-- at a6a41c1903db7f3b463b8ffbf0c3ecd227050a99 (2026-09-14), file bluebox_schema.sql. Bluebox is MIT licensed,
...
--
--   * postgis-extension
--     PGlite has no PostGIS; sql/bluebox-pglite-shim.sql stands in for it.
--   * postgis-comment
--     The COMMENT would fail without the extension.
--   * topology-schema
--     The topology schema belongs to PostGIS and stays empty otherwise.
--   * geography-typmod
--     The shim types geography as a domain over point, and domains take no
--     type modifier.
--   * pg_stat_statements
--     The test bed installs it at start up, before this file is loaded.
--   * film-replacement-cost
--     Upstream bug: get_overdue_rentals() and process_lost_inventory()
--     still read film.replacement_cost, a column that was replaced by
--     bluebox.get_replacement_cost(). Both routines error out as shipped.
--     Both uses sit in a select list, so the call keeps the column name
--     through the alias.
--   * duplicate-process-lost-rentals
--     Upstream bug: two overloads of process_lost_rentals() are both
--     callable with no arguments, so CALL bluebox.process_lost_rentals()
--     fails as "not unique". The two-argument one is the older of the pair
--     -- the procedure comment documents the three-argument version -- so
--     it goes.
--
...
-- [pglite] CREATE SCHEMA topology -- PostGIS only
...
CREATE FUNCTION bluebox.get_overdue_rentals(overdue_days integer DEFAULT 7) RETURNS TABLE(rental_id bigint, customer_id bigint, customer_name text, film_title text, rental_start timestamp with time zone, days_overdue integer, replacement_cost numeric)
    ...
    SELECT 
        ...
        bluebox.get_replacement_cost(f.film_id) AS replacement_cost
    FROM bluebox.rental r
